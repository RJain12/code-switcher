...
class ImportCwd(unittest.TestCase):
    def test_existing_project_is_reused_and_root_is_guarded(self):
        with tempfile.TemporaryDirectory() as tmp, patch.dict(os.environ):
            ...
            with patch.object(client, 't3_http', return_value={'projects': [{'id': 'existing', 'workspaceRoot': cwd}]}) as http, patch.object(client, 't3_rpc_value', return_value={'importedCount': 1, 'skippedCount': 0}) as rpc:
    ...
    def test_new_project_created_before_import_without_starting_turn(self):
        with tempfile.TemporaryDirectory() as tmp, patch.dict(os.environ):
            client = load(os.path.join(ROOT, 'codespace'), tmp)
            with patch.object(client, 't3_http', side_effect=[{'projects': []}, {}]) as http, patch.object(client, 't3_rpc_value', return_value={'importedCount': 0, 'skippedCount': 0}):
                result = client.t3_import_cwd(tmp, 'http://localhost', 'fixture')
